        ...
        if self.use_adapters:
            if self.use_tree_structure:
                ...
                while ind != 0:  # while we haven't found the root of the tree
                    ...
                    # print("\n" + str(ind))
                    adapters_to_activate.append(ind-1)
                    ind = self.domain_dict[ind]

                gp_output = self.adapter_module[adapters_to_activate[-1]](feed_forward_hidden_states)
                parent_output = self.adapter_module[adapters_to_activate[-2]](gp_output)
                leaf_output = self.adapter_module[adapters_to_activate[-3]](parent_output)
                feed_forward_hidden_states = leaf_output
