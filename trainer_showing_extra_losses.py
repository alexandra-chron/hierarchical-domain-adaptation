    ...
    def evaluation_loop(
        ...
        for ind, dataloader in enumerate(dataloaders):
            for step, inputs in enumerate(dataloader):
                ...
                loss_grandparent, _, _ = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys,
                ...
                if loss is not None:
                    ...
                    losses_host_leaf = losses_leaf if losses_host_leaf is None else torch.cat((losses_host_leaf, losses_leaf), dim=0)

                    losses_parent = self._nested_gather(loss_parent.repeat(batch_size))
                    losses_host_parent = losses_parent if losses_host_parent is None else torch.cat((losses_host_parent, losses_parent), dim=0)
                ...
                if self.args.eval_accumulation_steps is not None and (step + 1) % self.args.eval_accumulation_steps == 0:
                    ...
                    if losses_host_leaf is not None:
                        ...
                        all_losses_leaf = losses_leaf if all_losses_leaf is None else np.concatenate((all_losses_leaf, losses_leaf), axis=0)
                    if losses_host_parent is not None:
                        losses_parent = nested_numpify(losses_host_parent)
                        all_losses_parent = losses_parent if all_losses_parent is None else np.concatenate((all_losses_parent, losses_parent), axis=0)
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    loss, outputs = self.compute_loss(model, inputs, return_outputs=True, dataset_ind=dataset_ind, alt=alt, eval=eval)
