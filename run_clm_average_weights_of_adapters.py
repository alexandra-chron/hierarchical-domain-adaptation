...
def main():
    ...
    model_1 = GPT2LMHeadModel.from_pretrained(model_1_path, config=config,
                                            cache_dir=model_args.cache_dir)
    model_2 = GPT2LMHeadModel.from_pretrained(model_2_path, config=config,
                                            cache_dir=model_args.cache_dir)
    model_1_dict = model_1.state_dict()
    model_2_dict = model_2.state_dict()

    for key in model_1_dict:
        if "adapter" in key:
            model_2_dict[key] = (model_1_dict[key] + model_2_dict[key]) / 100

    model = GPT2LMHeadModel.from_pretrained(model_args.model_name_or_path, config=config,
                                            cache_dir=model_args.cache_dir)
    model.load_state_dict(model_2_dict)
