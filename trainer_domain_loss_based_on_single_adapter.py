...
class Trainer:
    ...
    def __init__(
        ...
        self.model = model
        self.eval = ["leaf", "parent", "grandparent"]
    ...
    def train(
        ...
        **kwargs,
        ...
        losses_per_domain_aggr = {}
        for i in range(len(train_dataloaders)):
            losses_per_domain_aggr[f"domain_{i}"] = torch.tensor(0.0).to(args.device)

        # _total_loss_scalar is updated everytime .item() has to be called on tr_loss and stores the sum of all losses
        self._total_loss_scalar = 0.0
        self._total_domain_loss_scalar = {}
        for i in range(len(train_dataloaders)):
            self._total_domain_loss_scalar[f"domain_{i}"] = 0.0
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, multi_batch in enumerate(epoch_iterator):
                ...
                for ind, inputs in enumerate(multi_batch):
                    # print("\n We are in domain {} !!!!!!!!!!!!!!!!!!!!!!!!!!!".format(ind))
                    # print("We activate {} adapter of this domain!!!!!!".format(self.eval[index]))
                    ...
                    if (
                        ...
                        loss = self.training_step(model, inputs, dataset_ind=ind + 1, alt=True, eval=self.eval[index])
        ...
        for i in range(len(losses_per_domain_aggr)):
            self._total_domain_loss_scalar[f"domain_{i}"] += losses_per_domain_aggr[f"domain_{i}"].item()
            metrics[f"train_loss_domain_{i}"] = self._total_domain_loss_scalar[f"domain_{i}"] / self.state.global_step
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval,  losses_per_domain=None):
        if self.control.should_log:
            ...
            if losses_per_domain:
                losses_per_domain_scalar = {}
                for i, (key, value) in enumerate(losses_per_domain.items()):
                    losses_per_domain_scalar[key] = value.item()
                    losses_per_domain[key] -= losses_per_domain[key]
                    logs[f"loss_domain_{i}"] = round(losses_per_domain_scalar[key] / (self.state.global_step - self._globalstep_last_logged), 4)
                    self._total_domain_loss_scalar[f"domain_{i}"] += losses_per_domain_scalar[f"domain_{i}"]
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], dataset_ind=None,
                      alt=False, eval=None) -> torch.Tensor:
        """
        ...
        if self.use_amp:
            with autocast():
                loss = self.compute_loss(model, inputs, dataset_ind=dataset_ind, alt=alt, eval=eval)
        else:
            loss = self.compute_loss(model, inputs, dataset_ind=dataset_ind, alt=alt, eval=eval)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, dataset_ind=None, alt=False, eval=None):
        """
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        outputs = model(**inputs, dataset_ind=dataset_ind, alt=alt, eval=eval)
    ...
    def evaluation_loop(
        ...
        preds_host = None
        labels_host = None
        losses_host_leaf = None
        losses_host_parent = None
        losses_host_grandparent = None

        # losses/preds/labels on CPU (final containers)
        all_losses = None
        all_losses_leaf = None
        all_losses_parent = None
        all_losses_grandparent = None
        ...
        for ind, dataloader in enumerate(dataloaders):
            for step, inputs in enumerate(dataloader):
                ...
                loss_leaf, _, _ = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys,
                                                       dataset_ind=ind + 1, alt=True, eval="leaf")
                loss_parent, _, _ = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys,
                                                         dataset_ind=ind + 1, alt=True, eval="parent")
                loss_grandparent, _, _ = self.prediction_step(model, inputs, prediction_loss_only,
                                                              ignore_keys=ignore_keys,
                ...
                if self.args.eval_accumulation_steps is not None and (
                    ...
                    if losses_host_leaf is not None:
                        losses_leaf = nested_numpify(losses_host_leaf)
                        all_losses_leaf = losses_leaf if all_losses_leaf is None else np.concatenate(
                            (all_losses_leaf, losses_leaf), axis=0)
                    if losses_host_parent is not None:
                        losses_parent = nested_numpify(losses_host_parent)
                        all_losses_parent = losses_parent if all_losses_parent is None else np.concatenate(
                            (all_losses_parent, losses_parent), axis=0)
                    if losses_host_grandparent is not None:
                        losses_grandparent = nested_numpify(losses_host_grandparent)
                        all_losses_grandparent = losses_grandparent if all_losses_grandparent is None else \
                            np.concatenate((all_losses_grandparent, losses_grandparent), axis=0)
            ...
            if all_losses_leaf is not None:
                all_losses_leaf = all_losses_leaf[:num_samples]
            if all_losses_parent is not None:
                all_losses_parent = all_losses_parent[:num_samples]
            if all_losses_grandparent is not None:
                all_losses_grandparent = all_losses_grandparent[:num_samples]
