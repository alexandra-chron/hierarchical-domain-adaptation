...
class Trainer:
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            # decay_parameters : all weights except for the biases
            decay_parameters = [name for name in decay_parameters if "bias" not in name]
            optimizer_grouped_parameters_old = [
                # {
            ...
            list_of_leaf_adapters = []
            list_of_parents = []
            list_of_grandparents = []
            list_of_rest = []
            list_of_biases = []
            for n, p in self.model.named_parameters():
                # print(n)
                if n in decay_parameters:
                    if "adapter_module.0" in n or "adapter_module.1" in n or "adapter_module.2" in n\
                            or "adapter_module.3" in n:
                        list_of_leaf_adapters.append(p)
                    elif "adapter_module.4" in n or "adapter_module.5" in n:
                        list_of_parents.append(p)
                    elif "adapter_module.6" in n:
                        list_of_grandparents.append(p)
                    else:
                        list_of_rest.append(p)
                else:
                    list_of_biases.append(p)
            optimizer_grouped_parameters = [{"name": "leaf_adapters", "params": list_of_leaf_adapters,
                                             "weight_decay": self.args.weight_decay},
                                            {"name": "parent_adapters", "params": list_of_parents,
                                             "weight_decay": self.args.weight_decay},
                                            {"name": "grandparent_adapters", "params": list_of_grandparents,
                                             "weight_decay": self.args.weight_decay},
                                            {"name": "weights", "params": list_of_rest,
                                             "weight_decay": self.args.weight_decay},
                                            {"name": "biases", "params":  list_of_biases,
                                             "weight_decay": 0.0}
                                            ]
            assert sum(p.numel() for g in optimizer_grouped_parameters for p in g['params']) \
                   == sum(p.numel() for g in optimizer_grouped_parameters_old for p in g['params'])
            ...
            for step, multi_batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    for param_group in self.optimizer.param_groups:
                        if param_group["name"] == "leaf_adapters":
                            reference_lr = param_group["lr"]
                        if param_group["name"] == "parent_adapters":
                            param_group["lr"] = reference_lr / 2
                        if param_group["name"] == "grandparent_adapters":
                            param_group["lr"] = reference_lr / 4
